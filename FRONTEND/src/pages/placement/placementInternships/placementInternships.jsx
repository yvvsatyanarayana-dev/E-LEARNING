/**
 * placementInternships.jsx — SmartCampus Internships Page
 ...
 *   GET    /placement/internships?limit=50   → list all drives/internships
 *   POST   /placement/internships            → create new internship (Add modal)
 *   DELETE /placement/internships/:id        → remove internship
...
function AddInternshipModal({ onClose, onAdd }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      // POST to /placement/internships — same endpoint as AddDriveModal in placementDashboard
      ...
      setApiErr(err.message ?? "Failed to create internship. Please try again.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="pi-panel">
        ...
        <div className="pi-header">
          <div className="pi-header-left">
            ...
            <div>
              <div className="pi-modal-title">Add Internship / Drive</div>
              <div className="pi-modal-sub">Register a new company internship</div>
            </div>
          </div>
        ...
        {success ? (
          ...
          <div className="pi-body">
            ...
            {step === 1 && (
              <div>
                <p className="af-section-desc">Enter the company and role details for this internship.</p>
            ...
            {step === 3 && (
              <div>
                ...
                <FSelect label="Internship Status" name="status" value={form.status} onChange={handle} options={STATUSES} required />
                <div className={`af-status-hint status-${form.status.toLowerCase()}`}>
                  ...
                  <span>
                    {form.status==="Ongoing"   ? "Internship is currently active." :
            ...
            <div className="pi-footer-right">
              ...
              {step < 3
                ...
                : <button className="btn btn-teal"  style={{fontSize:11,padding:"9px 22px"}} onClick={submit} disabled={saving}>
                    {saving ? <><div className="pd-spinner" />&nbsp;Saving…</> : <><CheckIco />Add Internship</>}
                  </button>
              }
            </div>
          </div>
    ...
    <Overlay onClose={onCancel}>
      <div className="pi-panel pi-panel-delete">
        ...
        <div className="pi-delete-sub">
          This will permanently remove the <strong style={{color:"var(--text)"}}>{intern.company}</strong> — {intern.role} internship.
        </div>
...
export default function PlacementInternships() {
  ...
  useEffect(() => {
    ...
    let raf;
    const loop = () => {
      rx.current+=(mx.current-rx.current)*0.14; ry.current+=(my.current-ry.current)*0.14;
  ...
  useEffect(() => {
    document.body.style.overflow = (showAdd || !!deleteTarget) ? "hidden" : "";
  ...
  useEffect(() => {
    const fetchAll = async () => {
      setLoading(true);
      try {
        const [meRes, statsRes, internshipsRes] = await Promise.allSettled([
          api.get("/auth/me"),
        ...
        if (internshipsRes.status === "fulfilled") {
          ...
          setInternships(raw.map(mapApiInternship));
        }
      } catch (err) {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      try {
        ...
        setInternships(arr.map(mapApiInternship));
      } catch (err) {
        console.error("Internship filter error:", err);
      }
    }, 350);
        ...
        <aside className="sidebar">
          ...
          <div className="sb-bottom">
            <div className="sb-pri">
              ...
              <div className="sb-pri-sub">AY 2024-25 · {loading?"…":`${internships.length} total`}</div>
              <div className="sb-pri-bar">
                <div className="sb-pri-fill" style={{ width: loading?"0%":`${internships.length ? Math.round(ppoCount/internships.length*100) : 0}%`, transition:"width 1s ease" }} />
              </div>
            </div>
        ...
        <div className="main">
          <header className="topbar">
            ...
            <div className="tb-search">
              ...
              <input type="text" placeholder="Search company, role, branch…" value={search} onChange={e=>setSearch(e.target.value)} style={{cursor:"none"}} />
            </div>
          ...
          <div className="content">
            <div className="greet-row">
              <div className="greet-tag"><div className="greet-pip"/><span className="greet-pip-txt">{loading?"Loading…":`${internships.length} Internships · AY 2024-25`}</span></div>
            ...
            {!loading && ppoCount > 0 && (
              <div className="ppo-banner">
                ...
                <div className="ppo-banner-info">
                  ...
                  <div className="ppo-banner-sub">These internships may result in Pre-Placement Offers upon completion.</div>
                </div>
            ...
            <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <span style={{marginLeft:"auto",fontSize:11,color:"var(--text3)"}}>{filtered.length} internship{filtered.length!==1?"s":""}</span>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:16}}>
              {loading ? (
                ...
                <div style={{gridColumn:"1/-1",textAlign:"center",padding:"60px 20px",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:14}}>
                  ...
                  <div style={{fontSize:12,color:"var(--text3)"}}>Try adjusting your search or filters, or add a new internship.</div>
                  <button className="btn btn-solid" style={{marginTop:16,fontSize:11,padding:"10px 20px"}} onClick={()=>setShowAdd(true)}>+ Add Internship</button>
                </div>
              ) : filtered.map(i => (

      ...
      <div className="app" ref={pageRef}>
        ...
        <div className="main">
          <header className="topbar">
            ...
            <span className="tb-page">Analytics</span>
          ...
          <main className="content">
            ...
            <div className="greet-row">
              <div className="greet-tag"><div className="greet-pip" /><span className="greet-pip-txt">Analytics Monitor</span></div>
              <h1 className="greet-title">Platform <em>Analytics.</em></h1>
            ...
            <div className="stat-grid">
              {stats.map((s, i) => (
                <div key={i} className={`stat-card ${s.accent}`} style={{ animationDelay:`${i * 80}ms`, cursor:"default" }}>
                  ...
                  <span className={`stat-delta delta-${s.dir}`}>{s.dir === "up" ? "↑" : s.dir === "dn" ? "↓" : "·"} {s.delta}</span>
                </div>
              ))}
            </div>

            {/* WEEKLY USAGE + MODULE USAGE */}
            <div className="main-grid-wide">
              <div className="panel">
                <div className="panel-hd">
                  <div className="panel-ttl"><I n="trend" size={15} /> Weekly Active Users</div>
                  <span style={{ fontSize:"10px", color:"var(--text3)" }}>Active sessions per day</span>
                </div>
                <div className="panel-body">
                  ...
                  <div className="usage-legend" style={{ marginTop:"16px" }}>
                    {[{color:"var(--indigo-l)",label:"Students" },{color:"var(--teal)",label:"Faculty" },{color:"var(--amber)",label:"Admin"}].map(l => (
                      <div key={l.label} className="ul-item"><div className="ul-dot" style={{ background:l.color }} />{l.label}</div>
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  {moduleUsage.map((m, i) => (
                    <div key={i} style={{ marginBottom:"13px" }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"5px", alignItems:"center" }}>
                        ...
                        <span style={{ fontSize:"10px", fontWeight:700, color:m.color }}>{m.pct}%</span>
                      </div>
                      <div className="dept-bar"><div className="dept-fill" data-width={`${m.pct}%`} style={{ width:0, background:m.color }} /></div>
                      <div style={{ fontSize:"9.5px", color:"var(--text3)", marginTop:"3px" }}>{m.enrolled} enrolled</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="panel">
              <div className="panel-hd">
                <div className="panel-ttl"><I n="bar" size={15} /> Department Engagement Scores</div>
                <span style={{ fontSize:"10px", color:"var(--text3)" }}>Avg across LMS, quizzes, attendance</span>
              </div>
              <div className="panel-body">
                  ...
                  {engagementScores.map((d, i) => (
                    <div key={i} style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:"12px", padding:"16px" }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"12px" }}>
                        <div>
                          <div style={{ fontSize:"16px", fontWeight:800, color:"var(--text)" }}>{d.dept}</div>
                          <div style={{ fontSize:"10px", color:"var(--text3)", marginTop:"2px" }}>{d.students} students</div>
                        </div>
                        <div style={{ fontFamily:"'Fraunces',serif", fontSize:"26px", color:d.color }}>{d.score}</div>
                      </div>
                      <div className="dept-bar" style={{ height:"4px" }}>
                        <div className="dept-fill" data-width={`${d.score}%`} style={{ width:0, background:d.color }} />
                      </div>
                      <div style={{ fontSize:"9px", color:"var(--text3)", marginTop:"5px" }}>Engagement score out of 100</div>
                    </div>
                  ))}
                </div>
              </div>
